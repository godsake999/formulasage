...
export default function ClientAllFormulas() {
  ...
  useEffect(() => {
    ...
    router.replace(`/formulas?${params.toString()}`); // ← FIXED missing backticks
  ...
  const filteredFormulas = useMemo(() => {
    ...
    return formulas.filter(
      (formula) =>
        ...
        formula.shortDescription[language].toLowerCase().includes(lowercasedTerm) ||
      ...
      {filteredFormulas.length > 0 ? (
        ...
        <div className="text-center col-span-full py-12">
          <p className="text-muted-foreground">
            {pageContent.allFormulas.noResults(searchParams.get('q') || '')}
          </p>
        </div>
      )}
    </div>
